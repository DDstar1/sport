...
def get_all_numbers_and_date_2(name_of_db):
    conn = sqlite3.connect(f'dbs/{name_of_db}')
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM data")
    rows = cursor.fetchall()
    return rows
...
def has_6_consecutive_losses_passed():
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT number FROM data ORDER BY ROWID DESC LIMIT 2")
        results = cursor.fetchall()
        values_list = [float(result[0]) for result in results]

        print(f"reuslts of consecutive losses are {results}")
        
        if all(value < 1.2 for value in values_list):
